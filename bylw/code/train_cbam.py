...
from models.resnet34_cbam import ResNet34_CBAM  #  导入CBAM模型
...
batch_size = 128
num_epochs = 20
model_names = ['resnet34_cbam']  # ✅ 指定CBAM模型名
...
def build_model(model_name):
    if model_name == 'resnet34_cbam':
        return ResNet34_CBAM(num_classes=num_classes)
    else:
...
def main():
    ...
    summary.to_csv(os.path.join(save_dir, 'model_comparison_cbam.csv'), index=False)
    print("\n 所有模型训练完成，结果保存至 model_comparison_cbam.csv")
